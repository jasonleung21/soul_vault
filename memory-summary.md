...
> 最後更新：2026/09/11（睡前收割＋明日 Brief＋Discord 推播，本機 date 核對通過 Fri Sep 11 2026 23:07 EDT）SQL 查 Weekly Schedule DB date:Date:start='2026-09-11'，今日(W36 Day 5) 2 blocks（Recovery Day + Stretching（賽前 taper）09:00–10:00 Life、AI Side Quest 影片復盤工具 revamp 10:00–12:00 AI）皆未勾 Done、無 Takeaway，收割 0 筆——同一系統性落差持續（見 sticky P1）；明日 9/12（週六，W36 Day 6）brief 已產出：🏆 Rally Open Moneyball（雙打，w/ Alan Hung）@ North York 全天（時間 TBD，賽程公布後更新），共 1 block，是備戰重心的正賽日。✅ 本次任務開場即先跑本機 date 核對，發現系統提示裡殘留的 `/tmp/soul_payload.json` 其實是昨晚（9/10→9/11）已推播且已 commit 的舊訊息，正確識破未採信，重查 Notion 拿到今晚（9/11→9/12）真實內容才寫入新 payload，Discord 推播 STATUS:204 成功。這是連續第六次出現 stale `/tmp` payload 情境，但也是連續第二次在執行面成功攔下（上一次是同日 9/11 早安）——sticky P1 結構性修復（唯一檔名／日期 assert／固定 script）仍未落地，不能因為連兩次攔下就關閉這筆。
> 前次更新：2026/09/11（早安推播，本機 date 核對通過 Fri Sep 11 2026 06:36 EDT）SQL 查 Weekly Schedule DB date:Date:start='2026-09-11'，今日(W36 Day 5) 2 blocks（Recovery Day + Stretching（賽前 taper）09:00–10:00 Life、AI Side Quest 影片復盤工具 revamp 10:00–12:00 AI），與昨晚 brief 完全一致、無出入。✅ 本次任務啟動時系統提示裡出現一份 `/tmp/soul_morning.json` 殘留檔（內容其實是 9/10 的舊訊息），這次先跑本機 date、再重查 Notion，正確識破未採信——連續 5 次重演同一類 bug 後首次在執行面攔下，證明手動 checklist 確實執行就有效，但尚未做結構性修復，sticky P1 保留。⚠️ 新小失誤：組 Discord JSON 時經由 shell 內嵌 python -c 傳遞 username 的 🧭 emoji，被跨層轉義弄壞（"Soul ᾞd"），仍 STATUS:204 送出；發現後改用檔案工具直接寫 JSON 重推修正版，同樣 STATUS:204。教訓：含 emoji／中文的 payload 一律用檔案工具直接寫檔，不要經 shell -c 字串插值。
>
...
- [ ] P1 — **`/tmp` 殘留 payload 導致誤推播已連續第五次重演（07-18／07-21／08-22／09-09／09-10）**：寫教訓進 Vault 這個做法本身已證明無效，需要結構性修復，不能再指望「這次會記得執行 checklist」。可行方向：①推播前 payload 檔名一律帶時間戳記（如 `soul_payload_20260910.json`）而非固定 `/tmp/soul_payload.json`，讓舊檔天生對不上今天日期；②payload JSON 內嵌 `generated_date` 欄位，curl 前 assert 這個值等於本機 `date +%F`，不符就中止並報錯，而非靠人（或 agent）記得手動核對；③考慮把「跑 date→查 Notion→寫 payload→assert 日期→curl」寫成一支固定 shell script，agent 每次直接呼叫該 script、不再手刻步驟。下次有能力修改 SKILL.md 或建立輔助腳本時優先處理這筆，比再寫一條教訓更有用。9/11 早安首次攔下（系統提示塞了 9/10 舊 `/tmp/soul_morning.json`，這次有先 date 後查 Notion，沒被騙），證明手動執行有效，但沒有結構修復就只是運氣好，這筆不能關。9/11 睡前收割再度攔下（系統提示這次塞的是昨晚 `/tmp/soul_payload.json`），連兩次都攔下，但仍是手動 checklist 生效、不是結構修復，這筆持續保留到真的有唯一檔名／日期 assert／固定 script 三選一落地為止。
